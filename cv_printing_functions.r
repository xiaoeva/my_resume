...
create_CV_object <-  function(data_location,
  ...
  if(is_google_sheets_location){
    ...
    cv$skills        <- read_gsheet(sheet_id = "computer_languages")
    cv$text_blocks   <- read_gsheet(sheet_id = "text_blocks")
    cv$contact_info  <- read_gsheet(sheet_id = "contact_info")
    cv$languages     <- read_gsheet(sheet_id="human_languages")
  } else {
...
#' @description Construct a bar chart of languages skills
#' @param out_of The relative maximum for skills. Used to set what a fully filled in skill bar is.
print_lang_skill_bars <- function(cv, out_of = 5, bar_color = "#969696", bar_background = "#d9d9d9", glue_template = "default"){
...
#' @description Slightly edited from print_section so links aren't sanitized
#' @param section_id ID of the entries section to be printed as encoded by the `section` column of the `entries` table
print_clips_section <- function(cv, section_id, glue_template = "default"){
